    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if isinstance(labels, torch.Tensor):
            ...
            if valid.numel() == 0 or valid.sum().item() == 0:
                logger.warning('all labels are -100 (no supervised tokens), replaced loss by 0 for stability')
                loss = torch.zeros((), device=loss.device if isinstance(loss, torch.Tensor) else labels.device)
        if isinstance(loss, torch.Tensor) and (torch.isnan(loss).any() or torch.isinf(loss).any()):
            logger.warning('loss is nan/inf, replaced by 0 for stability')
