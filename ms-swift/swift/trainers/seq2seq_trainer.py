        ...
        if labels is None:
            ...
            if compute_loss_func is not None:
                ...
                if isinstance(num_items_in_batch, torch.Tensor):
                    num_items = int(num_items_in_batch.item())
                else:
                    num_items = int(num_items_in_batch)
                if num_items <= 0:
                    logger.warning('num_items_in_batch==0 (no supervised tokens), replaced loss by 0 for stability')
